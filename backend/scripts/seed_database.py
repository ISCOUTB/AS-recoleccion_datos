            ...
            if table_exists(cursor, table):
                try:
                    ...
                    try:
                        ...
                        # No hacer rollback aquí, continuar con las demás tablas
            else:
...
def create_users(conn, cursor):
    ...
    users_data = [
        {
            ...
            "phone": "3001234567",
            "bio": "Estudiante de Ingeniería en Sistemas con interés en desarrollo web y móvil."
        },
        {
            ...
            "role": UserRole.STUDENT,
            "phone": "3109876543",
            "bio": "Apasionado por la electrónica y los sistemas embebidos."
        },
        {
            ...
            "role": UserRole.STUDENT,
            "phone": "3201234567",
            "bio": "Estudiante de psicología interesada en neurociencia y psicología educativa."
        },
        {
            ...
            "phone": "3151234567",
            "bio": "Docente con más de 10 años de experiencia en educación superior."
        },
        {
            ...
            "role": UserRole.ADMIN,
            "phone": "3001112233",
            "bio": "Administrador del sistema académico."
        }
    ]
    ...
    for user_data in users_data:
        try:
            ...
            if existing_user:
                print(f"Usuario {user_data['email']} ya existe, actualizando...")
                ...
                for key, value in user_data.items():
                    if key != "email":  # No actualizar el email
                ...
                # Añadir el ID del usuario al final
                ...
                conn.commit()
                print(f"Usuario {user_data['email']} actualizado")
            else:
                # Crear nuevo usuario
                ...
                cursor.execute(
                    ...
                    INSERT INTO users (
                        ...
                        student_id, program, semester, icfes_score, phone, bio,
            ...
            print(f"Error al crear/actualizar usuario {user_data.get('email')}: {e}")
            traceback.print_exc()
            conn.rollback()
...
def create_academic_records(conn, cursor, users):
    ...
    for user in users:
        try:
            ...
            if existing_record:
                print(f"Registro académico para {user['email']} ya existe, actualizando...")
                ...
                cursor.execute(
                    ...
                    SET period = %s, average_score = %s, credits_completed = %s, 
                        total_credits = %s, status = %s, updated_at = %s
                    WHERE id = %s
                    """,
                    (
                        "2025-1",
                ...
                conn.commit()
                print(f"Registro académico actualizado para {user['email']}")
            else:
            ...
            print(f"Error al crear/actualizar registro académico para {user['email']}: {e}")
            traceback.print_exc()
            conn.rollback()
...
def create_courses(conn, cursor):
    ...
    courses_data = [
        ...
        },
        {
            "code": "EST107",
            "name": "Estadística",
            "description": "Fundamentos de estadística y probabilidad",
            "credits": 3,
            "program": "General"
        },
        {
            ...
            "description": "Diseño y análisis de algoritmos y estructuras de datos",
            "credits": 4,
            "program": "Ingenieria en Sistemas"
        }
    ]
    ...
    for course_data in courses_data:
        try:
            ...
            if existing_course:
                print(f"Curso {course_data['code']} ya existe, actualizando...")
                ...
                cursor.execute(
                    """
                    UPDATE courses
                    SET name = %s, description = %s, credits = %s, program = %s
                    WHERE id = %s
                    """,
                    (
                        course_data["name"],
                ...
                conn.commit()
                print(f"Curso {course_data['name']} actualizado")
            else:
                # Crear nuevo curso
                cursor.execute(
                    """
            ...
            print(f"Error al crear/actualizar curso {course_data.get('code')}: {e}")
            traceback.print_exc()
            conn.rollback()
        ...
        try:
            ...
            for course in selected_courses:
                ...
                cursor.execute(
                    ...
                    WHERE user_id = %s AND course_id = %s AND period = %s
                    """,
                    (user["id"], course["id"], "2025-1")
                )
                existing_enrollment = cursor.fetchone()
                
                if existing_enrollment:
                    print(f"Matrícula para {user['email']} en {course['name']} ya existe, actualizando...")
                    ...
                    conn.commit()
                    print(f"Matrícula actualizada para {user['email']} en {course['name']}")
                else:
            ...
            print(f"Error al crear/actualizar matrícula para {user['email']}: {e}")
            traceback.print_exc()
            conn.rollback()
...
def create_survey(conn, cursor):
    ...
    try:
        ...
        if existing_survey:
            ...
            print(f"Ya existe una encuesta activa con ID {survey_id}, actualizando...")
            ...
            conn.commit()
            print(f"Encuesta actualizada: Encuesta de Prevencion de Desercion Estudiantil")
        else:
        ...
        # Verificar si ya existen preguntas para esta encuesta
        cursor.execute("SELECT COUNT(*) FROM questions WHERE survey_id = %s", (survey_id,))
        question_count = cursor.fetchone()[0]
        
        if question_count > 0:
            print(f"Ya existen {question_count} preguntas para esta encuesta, omitiendo creación de preguntas.")
        else:
        ...
        print(f"Error al crear/actualizar encuesta: {e}")
        traceback.print_exc()
        conn.rollback()
        return None
...
def create_notifications(conn, cursor, users):
    ...
    notification_messages = [
        ...
        "Tutoría programada para mañana",
        "Resultado de examen publicado",
    ...
    for user in users:
        try:
            ...
            for _ in range(num_notifications):
                ...
                if existing_notification:
                    ...
                    cursor.execute(
                        ...
                        SET type = %s, read = %s, created_at = %s
                        WHERE id = %s
                        """,
                        (
                            random.choice(notification_types),
                            random.random() > 0.7,  # 30% de probabilidad de estar leída
                            datetime.now() - timedelta(days=random.randint(0, 10)),
                            notification_id
                        )
                    )
                    conn.commit()
                    print(f"Notificación actualizada para {user['email']}")
                else:
            ...
            print(f"Error al crear/actualizar notificación para {user['email']}: {e}")
            traceback.print_exc()
            conn.rollback()

def create_support_tickets(conn, cursor, users):
    """Crear tickets de soporte para los usuarios"""
    issue_types = ["Técnico", "Académico", "Financiero", "Administrativo"]
    priorities = ["Alta", "Media", "Baja"]
    statuses = ["Abierto", "En proceso", "Resuelto", "Cerrado"]
    descriptions = [
        "No puedo acceder a mi cuenta",
        "Problemas con la matrícula de un curso",
        "Error al cargar calificaciones",
        "Solicitud de aplazamiento de semestre",
        "Problema con el pago de matrícula"
    ]
    
    for user in users:
        try:
            # Crear 0-2 tickets por usuario
            ...
            for _ in range(num_tickets):
                ...
                cursor.execute(
                    ...
                    INSERT INTO support_tickets (
                        user_id, issue_type, description, priority, status, 
                ...
                conn.commit()
                print(f"Ticket de soporte creado para {user['email']}")
        except Exception as e:
            print(f"Error al crear ticket de soporte para {user['email']}: {e}")
            traceback.print_exc()
            conn.rollback()
...
def main():
    ...
    try:
        ...
        create_support_tickets(conn, cursor, users)
